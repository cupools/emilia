    ...
    _createClass(Emilia, [{
        ...
        value: function _walkDecls(file, css) {
            ...
            css.walkDecls(this._traverseFilter(file, function (decl, file, group) {
                ...
                if (stat) {
                    if (tag !== inlineTag) {
                        _sprite2.default.add(tag, realpath, stat.ctime);
                    }
                } else {
                    decl.value = decl.value.replace(/\?__\w+/, '');
                    _log2.default.warn(url + ' not exsit');
                }
            }));
        }
    }, {
        ...
        value: function outputStyle(file) {
            ...
            var outputPath = _util2.default.resolve(opt.dest + name);
        ...
        value: function _getResource() {
            ...
            opt.src.map(function (f) {
                styles.push.apply(styles, _toConsumableArray(_glob2.default.sync(f).map(function (p) {
                    return _util2.default.resolve(p);
                })));
            });
        ...
        value: function _encode(realpath) {
            var base64 = _fsExtra2.default.readFileSync(realpath, { encoding: 'base64' });
            var name = _util2.default.basename(realpath);
