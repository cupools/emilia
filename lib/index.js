...
function emilia() {
  ...
  var assignGroup = function assignGroup(items) {
    return items.map(function (item) {
      return (0, _extends3.default)({}, item, { group: getGroup.bind(null, items)(item.tag) });
    });
  };
  var assignSprite = function assignSprite(items) {
    return items.map(function (item) {
      return (0, _extends3.default)({}, item, { sprite: getSprite.bind(null, processor, items)(item.group) });
    });
  };

  var result = map(assignSprite, assignGroup)(bundles);
}
