...
exports.default = {
    ...
    process: function process(store, options) {
        ...
        (0, _keys2.default)(styles).forEach(function (realpath) {
            var file = styles[realpath];
            file.content = (0, _postcss2.default)(_process.bind(null, store, file)).process(file.content).css;
        });
    }
};
...
function _process(store, file, root) {
    root.walkDecls(/background/, function (decl) {
        ...
        if (tag === INLINE) {
            // TODO, absolute url
            var _realpath = _path2.default.resolve(file.realpath, '..', url);
            var image = store.get(_realpath);
